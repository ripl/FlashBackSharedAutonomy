...
class LunarLander_v4(gym.Env, EzPickle):
    r"""
    ## Description
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        goal_dim = 2
        self.reach_idx = 0
        assert task in ['land_v1','land_v2','reach_v1','reach_v2']

        if task == 'reach_v2':
            # self.goal_dim = goal_dim * 3 + 1 ## all targets + current task idx 
            self.goal_dim = goal_dim + 3 ## 3 is one_hot for task index 
        else:
    ...
    def reset(
        ...
        *,
        ...
        if self.task in ['land_v1','land_v2']:
        ...
        ## Here we provide Lander v2
        if  self.task == 'land_v2':
            self.helipad_y = self.np_random.uniform(0,H/4)
        else:     
            self.helipad_y = H / 4
        ...
        if self.task == 'reach_v1':
            # Target
            radius = TARGET_R / SCALE
            # exclude middle half of the screen
            self.target_cx = self.np_random.uniform(radius, float(W) - radius)
            self.target_cy = self.np_random.uniform(H/2 + radius, H - radius)
            # self.target_cy = self.np_random.uniform(H/4, H - radius)
            # print('target', self.target_cx, self.target_cy)
        elif self.task == 'reach_v2':
            radius = TARGET_R / SCALE 
            self.target_cx = self.np_random.uniform(radius, float(W) - radius,size = (3,))
            target_cy = []
            for i in range(3):
                x = self.target_cx[i]
                chunk_index = int((x/float(W)) *( CHUNKS))
                chunk_index = int(np.clip(chunk_index,0,len(height)-1))
                min_height = smooth_y[chunk_index]
                y = self.np_random.uniform(min_height + radius, H-radius)
                target_cy.append(y)
        ...
        # Create Lander body
        #! TODO - Random initialization of the lander 
        if self.random_initialization:
            radius = TARGET_R / SCALE
            initial_y = VIEWPORT_H / SCALE
            initial_x = self.np_random.uniform(radius, VIEWPORT_W / SCALE - radius)
        else:
            initial_y = VIEWPORT_H / SCALE
        ...
        state, reward, terminated, done, info  = self.step(np.array([0, 0]) if self.continuous else 0)
        self.step_ct = 0
        return state,info 
    ...
    def step(self, action):
        ...
        reward = 0
        ### Shaping reward (optional)
        # shaping = (
        #     -500 * np.sqrt(state[0] * state[0] + state[1] * state[1])  # encourage low position error
        #     - 100 * np.sqrt(state[2] * state[2] + state[3] * state[3])  # encourage low speed
        #     - 100 * abs(state[4])
        #     + 10 * state[6]
        #     + 10 * state[7]
        ...
        if self.task in ['land_v1','land_v2']:
            ...
            target_cx = self.target_cx[self.reach_idx]
            target_cy = self.target_cy[self.reach_idx]
            _dx = (pos.x - target_cx) / (VIEWPORT_W / SCALE / 2)
            _dy = (pos.y - target_cy) / (VIEWPORT_H / SCALE / 2)
            shaping = (
                - 500*np.sqrt(_dx*_dx + _dy*_dy)
                - 100*np.sqrt(state[2]*state[2] + state[3]*state[3])
                - 100*abs(state[4])
        ...
        ### Penalize speed after first ground contact
        # if self.legs[0].ground_contact or self.legs[1].ground_contact:
        #     velocity_penalty = 1.0 * np.linalg.norm(state[2:4])
        #     velocity_penalty = np.clip(velocity_penalty, 0, 1)
        #     reward -= velocity_penalty

        ### Terminal reward
        terminated = False
        termination_info = None
        if self.max_episode_steps is not None and self.step_ct >= self.max_episode_steps:
            terminated = True 
            termination_info = 'time_out'
            reward -= 0.5
        else:
        ...
        if self.game_over or abs(state[0]) >= 1.0:
            ...
            if self.game_over:
                termination_info = 'crashed'
            else:
                termination_info = 'out_of_bound'
            self.game_over = True 
        ...
        if self.task == 'reach_v1':
            ...
            if np.sqrt(_dx*_dx + _dy*_dy) < TARGET_R / SCALE:
                ...
                terminated = True
                reward += 200
                termination_info = 'target_reached'

        if self.task == 'reach_v2':
            target_cx,target_cy = self.target_cx[self.reach_idx],self.target_cy[self.reach_idx]
            _dx, _dy = pos.x - target_cx, pos.y - target_cy
            if np.sqrt(_dx*_dx + _dy*_dy) < TARGET_R / SCALE:
                reward += 200 
                self.reach_idx = self.reach_idx + 1 
                
            if self.reach_idx == 3:
                ## Fix the task number for finite culmulative reward 
                terminated = True 
                self.reach_idx = 2 ## set back, don't move forward 
                termination_info = 'target_reached'

        if not self.lander.awake:
            # terminated = True
            # termination_info = 'landed'
            ...
            if at_site:
                reward += 500
                terminated = True
                termination_info = 'landed_at_site'
                self.lander.color1 = (0, 255, 0)
            else:
                reward += 0  # landed (neutral bonus)
        ...
        if self.task in ['land_v1','land_v2']:
            ...
            target_cy = self.target_cy[self.reach_idx]
            goal = [
                (target_cx - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2),
                (target_cy - VIEWPORT_H / SCALE /2) / (VIEWPORT_H / SCALE / 2),
            ]
            # goal = []
            # for i in range(3):
            #     goal.append((self.target_cx[i] - VIEWPORT_W / SCALE / 2) / (VIEWPORT_W / SCALE / 2))
            #     goal.append((self.target_cy[i] - VIEWPORT_H / SCALE / 2) / (VIEWPORT_H / SCALE / 2))
            # goal.append(self.reach_idx -1) ## to normalize in -1,0,1

            task_one_hot = [0.,0.,0.]
            task_one_hot[self.reach_idx] = 1
            goal = goal + task_one_hot
        ...
        info = {
            "goal": np.array(goal, dtype=np.float32) if goal is not None else None,
    ...
    def render(self):
        ...
        if self.task == 'reach_v1':
            ...
            color_list = [80,160,255]
            for i in range(3):
                pygame.draw.circle(self.surf, color=(color_list[i], 0, 0), center=(self.target_cx[i] * SCALE, self.target_cy[i] * SCALE), radius=TARGET_R)
